...
private const val CALENDAR_STATE_KEY = "CALENDAR_STATE_KEY"
...
class JDCalendar(context: Context, attrs: AttributeSet) : LinearLayout(context, attrs) {
    ...
    /**
     * Ensuring that when the screen is rotated the displayed month is
     * saved and recovered
     */
    override fun onSaveInstanceState(): Parcelable? {
        return Bundle().apply {
            putParcelable(SUPER_STATE_KEY, super.onSaveInstanceState())
            putLong(CALENDAR_STATE_KEY, baseAdapter.calendar.timeInMillis)
        }
    }
    ...
    override fun onRestoreInstanceState(state: Parcelable?) {
        ...
        if (newState is Bundle) {
            val calendarState = newState.getLong(CALENDAR_STATE_KEY)
            if (calendarState > 0L) {
                scope.launch {
                    val lastCalendar = Calendar.getInstance().fromMilis(calendarState)
                    baseAdapter.moveToCalendar(lastCalendar)
                    updateTopSection(lastCalendar)
                }
            }
